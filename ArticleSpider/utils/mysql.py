    ...
    def __getConn():
        ...
        if Mysql.__pool is None:
            __pool = PooledDB(MySQLdb, 5, host=Config.MYSQL_HOST, user=Config.MYSQL_USER, passwd=Config.MYSQL_PASSWORD,
                              db=Config.MYSQL_DBNAME, port=Config.MYSQL_DBPORT)
    ...
    def create_table_by_name(self, table_name=''):
        ...
        if ct < 1:
            create_sql = "create table " + table_name + "(id VARCHAR(64) PRIMARY key, " \
                                                        ...
                                                        "content VARCHAR(300), crawl_time datetime);"
            self._execute(create_sql)

    def judge_column_exist(self, table_name='', column_list=''):
        record_list = []
        columns = get_query_columns(column_list)
        query_sql = "SELECT COLUMN_NAME from information_schema.COLUMNS where " \
                    "TABLE_NAME = '" + table_name + "' AND COLUMN_NAME in (" + columns + ")"
        records = self._execute(query_sql)
